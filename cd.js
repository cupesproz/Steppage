  ...
  var flipTimer = function(element, options) {
    ...
    if (this.element.find('.seconds').length > 0) {
      this.options.seconds = this.element.find('.seconds')[0];
			console.log(this.options.seconds);
    }
  ...
  flipTimer.prototype = {
    ...
    renderDigits: function(subject, value) {
      ...
      if ($(subject).find('.digit').length == 0) {
        ...
        // set digit desc
        if (subject == _this.options.days) {
          digitdesc = "DAYS";
        } else if (subject == _this.options.hours) {
          digitdesc = "HOURS";
        } else if (subject == _this.options.minutes) {
          digitdesc = "MINUTES";
        } else if (subject == _this.options.seconds) {
          digitdesc = "SECONDS";
        } 

        // append two divs to contain two sets of digits for each subject
        $(subject).append('<div class="digit-set"></div><div class="digit-set"></div><div class="digit-desc">'+digitdesc+'</div>');
